...
impl Udp_Server {
    ...
    pub fn create(&mut self, ipaddr : &String, port : u32) -> i32 {
        ...
        unsafe {
            ...
            myaddr.sin_addr.s_addr = inet_addr(ipaddr.as_ptr() as *const libc::c_char);
            myaddr.sin_family = libc::AF_INET as u16;
            myaddr.sin_port = port.to_be() as u16;

            ret = libc::bind(self.fd,
                             &myaddr as *const libc::sockaddr_in as *const libc::sockaddr,
                             std::mem::size_of_val(&myaddr) as u32);
        }
